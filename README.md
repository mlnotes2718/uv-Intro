...
[uv Official Site](https://docs.astral.sh/uv/)
...
All uv projects contain the following files or folders when initialized:
...
- `.gitignore`: this is
- `main.py`: this is the main Python program

Before running uv, we need to perform `uv sync` to install all the necessary packages contain in `pyproject.toml`. The following files are created
...
- Only the file `pyproject.toml` is created when using `uv init --bare`.

Complete project folder looks like:
```text
.
├── .git
├── .venv
...
```bash
# Check if you install uv via brew
brew list

# For uv installing via brew
brew update
brew upgrade uv
...
Initialized new uv project and creating project folder at the same time
```bash
uv init <folder-name>                    # New project with git
uv init <folder-name> --python 3.11      # New project with specific Python version
```

Initialized new uv project on previously created folder
```bash
uv init                                  # Initialize in current folder
uv init --python 3.11                    # Initialize in current folder with specific Python version
```

The following files are created with `uv init`

```text
my-project/
├── pyproject.toml      ← project metadata and dependencies
├── .python-version     ← pins the Python version for this project
├── .git                ← initialized git with git folder
├── .gitignore          ← gitignore file that listed down those file that should not be uploaded to Github web portal.
├── main.py             ← the main python program file
└── README.md           ← document with description on this project
```
...
uv init my-project --package             # Creates project WITHOUT main.py but src folder is created
uv init my-project --bare                # Minimal setup: only pyproject.toml is created
```

**Key Differences:**
- **Default (`uv init`)**: Full project structure with `main.py`, git initialized, README.md created
- **`--bare`**: Minimal setup - only creates `pyproject.toml`

### Sync Environment
```bash
uv sync                                  # Ensures `.venv` matches `uv.lock`, create `.venv` and `uv.lock` if not there
uv sync --group docs                     # Install specific dependency group for building documentation
```
